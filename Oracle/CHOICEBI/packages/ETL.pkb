    ...
    begin
        ...
        (
                ...
                from
                    ...
                    CHOICEBI.V_FACT_MEMBER_MONTH_EXP a
                where line_of_business = v_lob_name and
        ...
        (
            ...
            from
                --choicebi.fact_member_month a,
                CHOICEBI.V_FACT_MEMBER_MONTH_EXP a,
                (select max(month_id) max_month_id from CHOICEBI.V_FACT_MEMBER_MONTH_EXP WHERE line_of_business = v_lob_name ) b
            where line_of_business = v_lob_name and
                to_date(a.month_id, 'yyyymm') between to_date(v_jan1_dt, 'yyyymmdd') and to_date(v_dec31_dt, 'yyyymmdd')
        ),
        auths_mbr as
    ...
    full join
            ...
            (
                ...
                (
                    ...
                    FROM  
                        ...
                        CHOICEBI.V_FACT_MEMBER_MONTH_EXP b 
                    WHERE line_of_business = v_lob_name) B
                    where
                ...
                (       /* get data for null MRN which can be populated by looking back at FMM with matching subscriber id and getting max monthid's MRN to populate */
                    ...
                    (select /*+ full(a) */ a.*, ROW_NUMBER() OVER (PARTITION BY a.subscriber_id ORDER BY month_id desc) AS seq
                      ...
                      --choicebi.fact_member_month a
                      CHOICEBI.V_FACT_MEMBER_MONTH_EXP a 
                      where line_of_business = v_lob_name) fmm
                    ) where seq = 1
                ) b
        ...
        (
            (
                ...
                (SELECT /*+ full(b) */ MONTH_ID, MRN, SUBSCRIBER_ID, BOROUGH, COUNTY_CODE 
                FROM  
                    --choicebi.FACT_MEMBER_MONTH b
                    CHOICEBI.V_FACT_MEMBER_MONTH_EXP b 
                WHERE line_of_business = v_lob_name) B
        ...
        (
                ...
                (
                    ...
                    from
                    (
                      select * from F_PAID_HRS_BY_SOURCE where line_of_business = 'MLTC' and SERVICE_DATE between to_date(v_begdt, 'yyyymmdd') and to_Date(v_enddt, 'yyyymmdd')
                    )
                    group by
...
--        order by para_ord_mrn, the_date
        ...
        SELECT /*+ * /
              ...
              t1.PAY_CYCLE_ID,
              t1.PAY_PROVIDER_ID,
                (SUM(CASE     WHEN t1.PAY_TYPE_PAYMENT ='A' THEN 0
                            when  t1.PAY_TYPE_PAYMENT='T' THEN  -1*( t1.PAY_HOURS+t1.PAY_MINUTES/60)
                            ELSE ( t1.PAY_HOURS+t1.PAY_MINUTES/60)
          ...
          GROUP BY b.mrn, b.case_num,
                   ...
                   t1.PAY_PROVIDER_ID
        ;
        xutl.out('temp_tppev -> ' || sql%rowcount);
        commit;
        DBMS_STATS.GATHER_TABLE_STATS('CHOICEBI', 'TEMP_TPPEV', '', 10);

        insert into temp_tpcharge_invoice
        (service_date
                    , pay_date
                    , chg_boro
                    , case_num
        ...
        select /*+ ordered use_hash(t1,t2,b) * /
              t1.visit_dte as service_date
                , t2.inv_dte as pay_date
                , t1.chg_boro
                , t1.case_no as case_num
                , b.mrn
                , (sum(t1.no_of_hours)) as sum_hours
        from (select /*+ no_merge  materialize   * / STAFF_ID,no_of_hours,visit_dte,inv_no,chg_boro,case_no,payor_code,CHGE_STTS  from dw_owner.TPCHG_CHARGE where payor_code in ('VCF')) t1,
            dw_owner.open_invoice t2,
            dw_owner.case_facts b
        where (t1.inv_no=t2.inv_no) and (t1.case_no=b.case_num) and t1.visit_dte between to_date(v_begdt, 'yyyymmdd') and to_Date(v_enddt, 'yyyymmdd')
                and t1.inv_no is not null
                and t2.inv_dte is not null
                and t1.payor_code in ('VCF')
                   and t1.STAFF_ID like 'V%'
                and t1.CHGE_STTS NOT IN ('DEL','CHD','NBT','NBV','AWO','NBS')
        group by t1.visit_dte
                , t2.inv_dte
                , t1.chg_boro
                , t1.case_no
                , b.mrn
        ;
        xutl.out('temp_tpcharge_invoice -> ' || sql%rowcount);
        commit;
        DBMS_STATS.GATHER_TABLE_STATS('CHOICEBI', 'TEMP_TPCHARGE_INVOICE', '', 10);
        ...
        select a.*, sysdate
         ...
         (
             (
                SELECT v_lob_name SOURCE, 'TPPVE' as data_source,service_Date
                    ,b.month_id, a.mrn,subscriber_id, county_code, borough, decode(b.month_id,null,0,1)  is_active
                    ,(SUM(adjustedhours)) AS paid_hours
                    ,(SUM(adjustedhours)) as paid_hours_HE
                FROM temp_tppev a, (SELECT * FROM  MSTRSTG.FACT_MEMBER_MONTH b WHERE line_of_business = v_lob_name) B
                where
                    a.mrn = b.mrn(+) and
                    to_char(a.service_Date,'yyyymm') = b.month_id(+)
                GROUP BY service_Date,b.month_id, a.mrn,subscriber_id, county_code, borough
             )
            union
            (
                select v_lob_name SOURCE ,'CHG' as data_source,  service_date
                    ,b.month_id, a.mrn,subscriber_id, county_code, borough, decode(b.month_id,null,0,1)  is_active
                    , (sum(sum_hours)) as paid_hours
                    , (sum(sum_hours)) as paid_hours_HE
                from temp_tpcharge_invoice a, (SELECT * FROM  MSTRSTG.FACT_MEMBER_MONTH b WHERE line_of_business = v_lob_name) B
                where
                    a.mrn = b.mrn(+) and
                    to_char(a.service_Date,'yyyymm') = b.month_id(+)
                group by service_Date,b.month_id, a.mrn,subscriber_id, county_code, borough
                    ...
                    (
                        ...
                        SELECT /*+ noparallel  leading(b, t1)*/
                          ...
                          FROM DW_OWNER.CASE_FACTS b,
                            dw_owner.TPPVE_PAYMENT t1
                          where PROGRAM IN ('VCF')
                              AND (t1.pay_case_no = b.case_num)
                              and t1.pay_visit_date between to_date(v_begdt, 'yyyymmdd') and to_date(v_enddt,'yyyymmdd')
                              AND t1.PAY_CO_CODE IN ('00110','00107')
                              AND t1.pay_id_no like 'V%'
                    ) a, (SELECT * FROM  
                ...
                from
                    (
                        select /*+ noparallel leading(t1,b, t2) use_hash(t1,t2,b)*/
                            ...
                            ,t1.no_of_hours as PAID_HOURS
                            ,t1.chge_amt PAY_AMOUNT
                        from (select /*+  materialize   */ case_no,no_of_hours,chg_boro,visit_dte,inv_no,payor_code,STAFF_ID,CHGE_STTS,CHGE_AMT,TWICE_DAILY_SEQ_NO from dw_owner.TPCHG_CHARGE where payor_code in ('VCF')) t1,
                            dw_owner.open_invoice t2,
                            dw_owner.case_facts b
                        where
                            (t1.inv_no=t2.inv_no)
                            and (t1.case_no=b.case_num)
                            and t1.visit_dte between to_date(v_begdt, 'yyyymmdd') and to_Date(v_enddt, 'yyyymmdd')
                            and t1.inv_no is not null
                            and t2.inv_dte is not null
                            and t1.payor_code in ('VCF')
                            and t1.STAFF_ID like 'V%'
    ...
    begin
        ...
        (
            (
                select  MERCER_DOC_YEAR , b.record_id
                    , b.risk_group
                ...
                -- From Table 6.6 in Mercer Documentation        
                 ...
                 , CASE case when BOWELCONTINENCE in(1,2,3,4,5) and NEUROLOGICALQUADRAPLEGIA in (1,2,3) then 1
                        else 0
                 ...
                 , CASE case when (NEUROLOGICALALZHEIMERS in(1,2,3) or NEUROLOGICALDEMENTIA in(1,2,3)) and ADLTOILETTRANSFER=1 then 1 
                           else 0
   ...
   then
    ...
    begin
        ...
        (
            ...
            order by a.patient_id, a.created_date desc
        ),
        ass_data as
            (
        ...
        select
                    ...
                    , (select DL_LOB_GROUP_ID from mstrstg.d_line_of_business where upper(lob_name) = (case when substr(l.client_patient_id, 1,2)='V6' then 'FIDA' else 'MLTC' end) ) lob_id
        ...
        order by l.created_on,l.unique_id
    );
