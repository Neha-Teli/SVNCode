DROP VIEW V_QUALITY_MSR_REASS_INCOMPLETE;

CREATE OR REPLACE VIEW V_QUALITY_MSR_REASS_INCOMPLETE
(
    MSR_TYPE,
    MSR_ID,
    MSR_TOKEN,
    ...
    WITH m AS
         ...
         b AS
             (SELECT a.*,
                     ...
                     CASE
                         ...
                         WHEN ADD_MONTHS(b.assessmentdate, 6) > period_end_date THEN null
         ...
         c AS
             (SELECT b.*,
                     CASE   WHEN due_month is null THEN 'N' 
                            WHEN b.Period_start_Date = b.Period_start_Date2 AND b.Period_end_Date = b.Period_end_Date2 THEN 'Y' ELSE 'N' END
                      AS eligible_ind,
         ...
         QIP AS
             (SELECT A.*,
                     ...
                     1 DENOMINATOR
              FROM   QIP_REASSESSMENT A, choicebi.DIM_QUALITY_MEASURES B
