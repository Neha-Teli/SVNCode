DROP VIEW V_HHA_HOURS_RISK_SCORES_STD30;

CREATE OR REPLACE VIEW V_HHA_HOURS_RISK_SCORES_STD30
(
    MONTH_ID,
    LOB,
    REGION,
    ...
    SELECT /*+ use_hash(a b c) */
         ...
         /*C.N_MBRS_DISENROLLEES1,
         C.RISK_SCORE_DISENROLLEES1,
         C.ORDER_AUTH_HRS_DISENROLLEES1,
         C.PAID_HRS_DISENROLLEES1,
         C.N_MBRS_DISENROLLEES3,
         C.RISK_SCORE_DISENROLLEES3,
         C.ORDER_AUTH_HRS_DISENROLLEES3,
         C.PAID_HRS_DISENROLLEES3*/
    ...
    FROM group_criteria a
         LEFT JOIN v_hha_zuno2_enroll_non_ltp_std30 b ON (a.month_id = b.month_id AND nvl(a.REGION,'NA') = nvl(b.REGION,'NA') AND a.county = b.county AND a.lob = b.lob)
         LEFT JOIN v_hha_zuno2_disenroll_non_ltp_STD30 c ON (a.month_id = c.month_id AND nvl(a.REGION,'NA') = nvl(c.REGION,'NA') AND a.county = c.county AND a.lob = c.lob);


GRANT SELECT ON V_HHA_HOURS_RISK_SCORES_STD30 TO LINKADM;

GRANT SELECT ON V_HHA_HOURS_RISK_SCORES_STD30 TO ROC_RO;



select * from V_HHA_HOURS_RISK_SCORES_STD30
