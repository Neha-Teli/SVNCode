DROP VIEW CHOICEBI.FACT_IP_ADDMISSION_DATA;

CREATE OR REPLACE FORCE VIEW CHOICEBI.FACT_IP_ADDMISSION_DATA
 AS
    WITH IN01 AS
             (SELECT /* materialized driving_site(a) cardinality(a 400000) cardinality(b 400000) cardinality(c 400000) cardinality(d 400000) */
              ...
              FROM   tmg.Cmc_clhp_hosp@nexus2 A
                     JOIN tmg.Cmc_clcl_claim@nexus2 b
                         ON a.clcl_id = b.clcl_id
                     LEFT JOIN TMG.CMC_prpr_prov@nexus2 C
                         ON b.prpr_id = c.prpr_id
                     LEFT JOIN tmg.Cmc_clmd_diag@nexus2 D
                         ON     a.clcl_id = D.clcl_id
                            AND d.clmd_type = 'AD' /* this pulls the admitting dx*/
                     LEFT JOIN tmg.Cmc_clmd_diag@nexus2 E
                         ON     a.clcl_id = e.clcl_id
                            AND e.clmd_type = '01' /* this pulls the primary dx*/
                     LEFT JOIN tmg.cmc_meme_member@nexus2 F
                         ON a.meme_ck = f.meme_ck
                     LEFT JOIN tmg.cmc_sbsb_subsc@nexus2 G
                         ON f.sbsb_ck = g.sbsb_ck
                     LEFT JOIN tmg.Cmc_mctr_cd_trans@nexus2 H
                         ON     a.CLHP_DC_STAT = h.mctr_value
                            AND h.MCTR_TYPE = 'DCST'
                     LEFT JOIN
                     tmg.Cmc_idcd_diag_cd@nexus2 I
                         ON     d.idcd_id = i.idcd_id
                            AND a.CLHP_STAMENT_FR_DT BETWEEN i.IDCD_EFF_DT
                                                         AND i.IDCD_TERM_DT
                     LEFT JOIN
                     tmg.Cmc_idcd_diag_cd@nexus2 j
                         ON     e.idcd_id = j.idcd_id
                            AND a.CLHP_STAMENT_FR_DT BETWEEN j.IDCD_EFF_DT
                                                         AND j.IDCD_TERM_DT
                     JOIN
                     TMG.Cmc_mepe_prcs_elig@nexus2 K
                         ON     A.MEME_CK = K.MEME_CK
                            AND a.CLHP_STAMENT_TO_DT BETWEEN MEPE_EFF_DT
                                                         AND MEPE_TERM_DT
              WHERE      k.pdpd_id NOT IN
                     ...
                     AND trunc(CLHP_STAMENT_FR_DT) >= '01-NOV-2014'
                     AND CLCL_INPUT_DT <= trunc(SYSDATE) - 2
              UNION ALL
              SELECT /* materialized driving_site(a) cardinality(a 400000) cardinality(b 400000) cardinality(c 400000) cardinality(d 400000) */
                    DISTINCT g.sbsb_id,
                             a.clcl_id,
              ...
              FROM   TMG_FIDA.Cmc_clhp_hosp@nexus2 A
                     JOIN TMG_FIDA.Cmc_clcl_claim@nexus2 b
                         ON a.clcl_id = b.clcl_id
                     LEFT JOIN TMG_FIDA.CMC_prpr_prov@nexus2 C
                         ON b.prpr_id = c.prpr_id
                     LEFT JOIN TMG_FIDA.Cmc_clmd_diag@nexus2 D
                         ON     a.clcl_id = D.clcl_id
                            AND d.clmd_type = 'AD' /* this pulls the admitting dx*/
                     LEFT JOIN TMG_FIDA.Cmc_clmd_diag@nexus2 E
                         ON     a.clcl_id = e.clcl_id
                            AND e.clmd_type = '01' /* this pulls the primary dx*/
                     LEFT JOIN TMG_FIDA.cmc_meme_member@nexus2 F
                         ON a.meme_ck = f.meme_ck
                     LEFT JOIN TMG_FIDA.cmc_sbsb_subsc@nexus2 G
                         ON f.sbsb_ck = g.sbsb_ck
                     LEFT JOIN TMG_FIDA.Cmc_mctr_cd_trans@nexus2 H
                         ON     a.CLHP_DC_STAT = h.mctr_value
                            AND h.MCTR_TYPE = 'DCST'
                     LEFT JOIN
                     TMG_FIDA.Cmc_idcd_diag_cd@nexus2 I
                         ON     d.idcd_id = i.idcd_id
                            AND a.CLHP_STAMENT_FR_DT BETWEEN i.IDCD_EFF_DT
                                                         AND i.IDCD_TERM_DT
                     LEFT JOIN
                     TMG_FIDA.Cmc_idcd_diag_cd@nexus2 j
                         ON     e.idcd_id = j.idcd_id
                            AND a.CLHP_STAMENT_FR_DT BETWEEN j.IDCD_EFF_DT
                                                         AND j.IDCD_TERM_DT
                     JOIN
                     TMG_FIDA.Cmc_mepe_prcs_elig@nexus2 K
                         ON     A.MEME_CK = K.MEME_CK
                            AND a.CLHP_STAMENT_TO_DT BETWEEN MEPE_EFF_DT
                                                         AND MEPE_TERM_DT
              WHERE      k.pdpd_id NOT IN
         ...
         IN03 AS
             (SELECT /* materialized driving_site(a) cardinality(a 400000) cardinality(b 400000) */
              ...
              FROM   VNS_CHOICE1.Value_options@nexus2 a
                     JOIN tmg.cmc_sbsb_subsc@nexus2 B
                         ON SUBSTR(a.MEMBNO, 1, 9) = b.sbsb_id
                     JOIN tmg.cmc_meme_member@nexus2 C
                         ON b.sbsb_ck = c.sbsb_ck
                     LEFT JOIN in02 D ON a.DISTAT = D.code
                     LEFT JOIN
                     tmg.Cmc_idcd_diag_cd@nexus2 E
                         ON     DIAGN1 = e.idcd_id
                            AND a.HOSBEG BETWEEN e.IDCD_EFF_DT
                                             AND e.IDCD_TERM_DT
              WHERE      LINENO = '001'
                     AND ALWAMT > 0
                     AND (   BILCOD LIKE '11%'
              ...
              FROM   VNS_choice1.VALUE_OPTIONS_FIDA@nexus2 a
                     JOIN TMG_FIDA.cmc_sbsb_subsc@nexus2 B
                         ON SUBSTR(a.MEMBNO, 1, 9) = b.sbsb_id
                     JOIN TMG_FIDA.cmc_meme_member@nexus2 C
                         ON b.sbsb_ck = c.sbsb_ck
                     LEFT JOIN in02 D ON a.DISTAT = D.code
                     LEFT JOIN
                     TMG_FIDA.Cmc_idcd_diag_cd@nexus2 E
                         ON     DIAGN1 = e.idcd_id
                            AND a.HOSBEG BETWEEN e.IDCD_EFF_DT
                                             AND e.IDCD_TERM_DT
              WHERE      LINENO = '001'
...
--                             NULL CLCL_TOT_PAYABLE
              FROM   tmg.Cmc_umin_inpatient@nexus2 A
                     JOIN
                     tmg.Cmc_mepe_prcs_elig@nexus2 B
...
--                             NULL CLCL_TOT_PAYABLE
         ...
         IN01_c AS
             (SELECT *
              FROM   IN01_b a
              WHERE  (meme_ck, TRUNC(CLHP_STAMENT_FR_DT)) 
          NOT IN (SELECT meme_ck, TRUNC(CLHP_STAMENT_FR_DT) FROM   IN01 b)),
         IN01_d AS
             (SELECT DISTINCT a.*
              FROM   IN01_c A
                     JOIN in01 B ON     a.meme_ck = b.meme_ck AND a.CLHP_STAMENT_FR_DT BETWEEN b.previous_dt AND b.after_dt),
         IN01_e AS
         ...
         in07_d AS
             (SELECT DISTINCT
              ...
              FROM   tmg.Cmc_mepe_prcs_elig@nexus2 A
                     JOIN tmg.cmc_meme_member@nexus2 B ON a.meme_ck = b.meme_ck
                     JOIN tmg.cmc_sbsb_subsc@nexus2 C ON b.sbsb_ck = c.sbsb_ck
              ...
              FROM   (SELECT sbsb_id,
                      ...
                      FROM   (                                  /*  in07_f  */
                              SELECT sbsb_id,
                                     ...
                                     lob,
                                     plan
                              FROM   (SELECT c.* --       ,c.sbsb_id, c.meme_last_name, meme_first_name, eff_dt, term_date
                        ...
                        from ( 
                                select b.*
                        --        , case when diff is null or diff > 1 then eff_dt else null end as temp
                                  , sum(case when diff is null then 0
                                             when diff in(1) then 0
                                             when diff >= 2 then 1
                                        end) over (partition by sbsb_id order by eff_dt) +1  as member_adm_seq
                                from ( 
                                        select a.*
                                               , lag(eff_dt, 1) over (partition by sbsb_id order by eff_dt, term_date) as lag_adm_dt
                                               , lag(term_date, 1) over (partition by sbsb_id order by eff_dt, term_date) as lag_dis_dt  
                        --                       , row_number() over (partition by sbsb_id order by eff_dt, term_date) as seq
...
(
    select in08a.*
    , to_date(to_char(CLHP_STAMENT_TO_DT,'YYYYMM') || '01','YYYYMMDD') as mos2
    from in08a 
    where active =1
),
in09 AS
(                                                      /* in09 */
              SELECT * FROM   
          (
                  SELECT a.*, TO_DATE(TO_CHAR(CLHP_STAMENT_FR_DT, 'YYYYMM')|| '01', 'YYYYMMDD') AS month_id,
                   1 as count 
                   , case when CLHP_STAMENT_TO_DT > sysdate then null
                          when CLHP_STAMENT_FR_DT=CLHP_STAMENT_TO_DT then 1 
                          when CLHP_STAMENT_TO_DT<CLHP_STAMENT_FR_DT then null
                          else CLHP_STAMENT_TO_DT-CLHP_STAMENT_FR_DT
                     end as los
    ...
    SELECT a.MONTH_ID,
           ...
           LOS
    FROM   in09 a
           LEFT JOIN FACT_MEMBER_MONTH B
               ON (    A.SBSB_ID = B.SUBSCRIBER_ID
                   AND TO_CHAR(A.MONTH_ID, 'YYYYMM') = b.month_id);


GRANT SELECT ON CHOICEBI.FACT_IP_ADDMISSION_DATA TO DW_OWNER;

GRANT SELECT ON CHOICEBI.FACT_IP_ADDMISSION_DATA TO MSTRSTG;
