DROP VIEW CHOICEBI.V_IP_SNF_DATA;

CREATE OR REPLACE FORCE VIEW CHOICEBI.V_IP_SNF_DATA
AS      
WITH IN01 AS
             (SELECT /*+ materialized driving_site(a) cardinality(a 400000) cardinality(b 400000) cardinality(c 400000) cardinality(d 400000) */
                    DISTINCT
                     ...
                     a.CLHP_DC_STAT,
                     h.MCTR_DESC,
                     CLHP_STAMENT_FR_DT - 1 previous_dt,
                     CLHP_STAMENT_FR_DT + 1 after_dt,
                     CLHP_FAC_TYPE ||
                     CLHP_BILL_CLASS ||
                     CLHP_FREQUENCY
                         AS Billtype
              FROM   tmg.Cmc_clhp_hosp@nexus2 A
                     ...
                     JOIN tmg.cmc_cdml_cl_line b2
                         ON     b.clcl_id = b2.clcl_id
                            AND RCRC_id = '0022' /* this determines that this is the SNF stay*/
              ...
              SELECT /*+ materialized driving_site(a) cardinality(a 400000) cardinality(b 400000) cardinality(c 400000) cardinality(d 400000) */
                    DISTINCT g.sbsb_id,
                             ...
                             j.IDCD_DESC AS Prim_dx_desc,
                             K.pdpd_id,
                             --case when substr(K.pdpd_id,1,2)='VN' then MA when substr(k.pdpd_id,1,2)='MD'  then SH else  end as Plan,
                             'FIDA' PLAN,
                             a.CLHP_DC_STAT,
                             h.MCTR_DESC,
                             ...
                             CLHP_FREQUENCY
                                 AS Billtype
              FROM   TMG_FIDA.Cmc_clhp_hosp A
                     JOIN TMG_FIDA.Cmc_clcl_claim@nexus2 b
                         ON a.clcl_id = b.clcl_id
                     JOIN TMG_FIDA.cmc_cdml_cl_line b2
                         ON     b.clcl_id = b2.clcl_id
                            AND RCRC_id = '0022'
              ...
              WHERE      k.pdpd_id NOT IN
                     ...
                     AND CLHP_STAMENT_FR_DT >= '01-NOV-2014'),
         ...
         IN01_b AS
             (SELECT /*+ materialized driving_site(a) cardinality(a 400000) cardinality(b 400000) */
              ...
              WHERE      B.pdpd_id NOT IN
                     ...
                     AND a.UMIN_ACT_ADM_DT >= '01-NOV-2014'
                     AND UMIN_INPUT_DT <= SYSDATE - 2
              UNION ALL
              SELECT /*+ materialized driving_site(a) cardinality(a 400000) cardinality(b 400000) */
                    DISTINCT d.sbsb_id,
                             a.meme_ck,
         ...
         in07_d AS
             (SELECT DISTINCT
              ...
              WHERE  MEPE_ELIG_IND = 'Y'),
         in07_e AS
             (SELECT sbsb_id, (Eff_dt) AS Eff_dt, (term_date) AS term_date
              --, totpd
              FROM   (SELECT   c.sbsb_id,
         ...
         in07_f AS
             (SELECT   A.sbsb_id,
                       Eff_dt,
                       term_date,
                       MIN((Eff_dt)) new_adm_dt,
                       MAX((term_date)) AS new_dis_dt
              FROM     in07_e A
              GROUP BY sbsb_id, Eff_dt, term_date),
         in07_h AS                                                 --in07_g as
             (SELECT sbsb_id, Eff_dt, term_date
              FROM   (SELECT sbsb_id,
                             new_adm_dt Eff_dt,
                             new_dis_dt term_date,
                             ROW_NUMBER()
                             OVER (PARTITION BY sbsb_id
         ...
         in08 AS
             (SELECT DISTINCT
                     ...
                     END
                         AS active,
                     ROW_NUMBER()
                     OVER (PARTITION BY a.sbsb_id, a.CLHP_STAMENT_TO_DT
                           ORDER BY a.sbsb_id, a.CLHP_STAMENT_TO_DT)
                         seq
              FROM   in07_ca A
                     LEFT JOIN in05 B
                         ON     a.sbsb_id = b.sbsb_id
         ...
         in09 AS
             (SELECT *
              FROM   
              (SELECT a.*,TO_DATE(TO_CHAR(CLHP_STAMENT_FR_DT, 'YYYYMM')||
                                     '01',
                                     'YYYYMMDD')
                                 AS month_id,
                             1 AS COUNT,
              ...
              WHERE      month_id >= TO_DATE('01jan2015')
                     AND month_id <=   TO_DATE(TO_CHAR(SYSDATE, 'YYYYMM')||
                                               '01',
                                               'YYYYMMDD')
