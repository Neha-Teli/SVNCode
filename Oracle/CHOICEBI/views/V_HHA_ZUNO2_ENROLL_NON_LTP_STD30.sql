DROP VIEW V_HHA_ZUNO2_ENROLL_NON_LTP_STD30;

CREATE OR REPLACE VIEW V_HHA_ZUNO2_ENROLL_NON_LTP_STD30
(
    MONTH_ID,
    LOB,
    REGION,
...
with months as
    ...
    (SELECT   m.month_id,
              ...
              ROUND( AVG(ordered_authed_hrs_month_std30), 1) AS order_auth_hrs --    , round(avg(authed_hrs_month_std30),1) as auth_hrs
                                                                              ,
              ROUND( AVG(paid_hrs_month_std30), 1) AS paid_hrs --New Enrollee this month
                                                              ,
              ...
              ROUND( AVG(CASE WHEN enrolled_flag = 1 THEN ordered_authed_hrs_month_std30 END), 1) AS order_auth_hrs_New_Enrollees1 --    , round(avg(case when enrolled_flag=1 then authed_hrs_month_std30 end),1) as auth_hrs_New_Enrollees1
                                                                                                                                  ,
              ROUND( AVG(CASE WHEN enrolled_flag = 1 THEN paid_hrs_month_std30 END), 1) AS paid_hrs_New_Enrollees1,
              ...
              ROUND( AVG(CASE WHEN enrolled_flag = 0 THEN ordered_authed_hrs_month_std30 END), 1) AS order_auth_hrs_All_Others1 --    , round(avg(case when enrolled_flag=0 then authed_hrs_month_std30 end),1) as auth_hrs_All_Others1
                                                                                                                               ,
              ROUND( AVG(CASE WHEN enrolled_flag = 0 THEN paid_hrs_month_std30 END), 1) AS paid_hrs_All_Others1 --New enrollee in past 3 months
                                                                                                               ,
              ...
              ROUND( AVG(CASE WHEN months_enrolled IN (1, 2, 3) THEN ordered_authed_hrs_month_std30 END), 1) AS order_auth_hrs_New_Enrollees3 --    , round(avg(case when months_enrolled in(1,2,3) then authed_hrs_month_std30 end),1) as auth_hrs_New_Enrollees3
                                                                                                                                             ,
              ROUND( AVG(CASE WHEN months_enrolled IN (1, 2, 3) THEN paid_hrs_month_std30 END), 1) AS paid_hrs_New_Enrollees3,
              ...
              ROUND( AVG(CASE WHEN months_enrolled > 3 THEN ordered_authed_hrs_month_std30 END), 1) AS order_auth_hrs_All_Others3 --    , round(avg(case when months_enrolled >3 then authed_hrs_month_std30 end),1) as auth_hrs_All_Others3
                                                                                                                                 ,
              ROUND( AVG(CASE WHEN months_enrolled > 3 THEN paid_hrs_month_std30 END), 1) AS paid_hrs_All_Others3
--     FROM     v_hha_zuno2_raw_enroll
     FROM     months m left join v_hha_zuno2_raw_enroll a on (a.month_id  = m.month_id and A.LOB = m.lob and A.REGION = m.region and a.county = m.county and ltp_ind = 0 AND m.month_id >= 201701)
     GROUP BY m.month_id)
    UNION
    (SELECT   m.month_id,
              '1-OVERALL' AS LOB,
              ...
              ROUND( AVG(CASE WHEN months_enrolled > 3 THEN paid_hrs_month_std30 END), 1) AS paid_hrs_All_Others3
--     FROM     v_hha_zuno2_raw_enroll
--     WHERE    ltp_ind = 0 AND month_id >= 201701
     FROM     months m left join v_hha_zuno2_raw_enroll a on (a.month_id  = m.month_id and A.LOB = m.lob and A.REGION = m.region and a.county = m.county and ltp_ind = 0 AND m.month_id >= 201701)
