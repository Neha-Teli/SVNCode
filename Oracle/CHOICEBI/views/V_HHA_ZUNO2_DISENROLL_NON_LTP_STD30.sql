DROP VIEW V_HHA_ZUNO2_DISENROLL_NON_LTP_STD30;

CREATE OR REPLACE VIEW V_HHA_ZUNO2_DISENROLL_NON_LTP_STD30
(
    MONTH_ID,
    LOB,
    REGION,
    ...
    WITH PREV_MONTH_DISENROLL AS
             (SELECT /*+ NO_MERGE MATERIALIZE */
                    MONTH_ID,
                       ...
                       SUM(ordered_authed_hrs_month_std30) ordered_authed_hrs_month,
                       SUM(paid_hrs_month_std30) AS paid_hrs
    ...
    (SELECT   m.month_id,
              ...
              ROUND( AVG(CASE WHEN disenrolled_flag = 1 THEN ordered_authed_hrs_month_std30 END), 1) AS order_auth_hrs_disenrollees1,
              ROUND( AVG(CASE WHEN disenrolled_flag = 1 THEN paid_hrs_month_std30 END), 1) AS paid_hrs_disenrollees1,
...
--     FROM     v_hha_zuno2_raw_disenroll a
--     WHERE    1 = 1 AND ltp_ind != 1 AND MONTHS_DISENROLLED <= 1 AND month_id >= 201701
...
--     WHERE    1 = 1 AND ltp_ind != 1 AND MONTHS_DISENROLLED <= 1 AND month_id >= 201701
    ...
    (SELECT   m.month_id,
              ...
              (SELECT SUM(N_Mbrs_PREV)
               FROM   PREV_MONTH_DISENROLL B
               WHERE  (B.MONTH_ID BETWEEN TO_CHAR( ADD_MONTHS( TO_DATE( m.MONTH_ID, 'YYYYMM'), -2), 'YYYYMM') AND m.MONTH_ID
                      ) AND b.LOB = m.LOB AND m.region = b.region)
                  AS N_Mbrs_disenrollees3,
              (SELECT ROUND( SUM(Risk_score) / SUM(N_Mbrs_PREV), 3)
               FROM   PREV_MONTH_DISENROLL B
               WHERE  (B.MONTH_ID BETWEEN TO_CHAR( ADD_MONTHS( TO_DATE( m.MONTH_ID, 'YYYYMM'), -2), 'YYYYMM') AND m.MONTH_ID
...
--     WHERE    1 = 1 AND ltp_ind != 1 AND MONTHS_DISENROLLED <= 1 AND month_id >= 201701
    ...
    (SELECT   m.month_id,
              ...
              (SELECT SUM(N_Mbrs_PREV)
               FROM   PREV_MONTH_DISENROLL B
               WHERE  (B.MONTH_ID BETWEEN TO_CHAR( ADD_MONTHS( TO_DATE( m.MONTH_ID, 'YYYYMM'), -2), 'YYYYMM') AND m.MONTH_ID
                      ) AND b.county = m.county AND m.region = b.region)
                  AS N_Mbrs_disenrollees3,
              (SELECT ROUND( SUM(Risk_score) / SUM(N_Mbrs_PREV), 3)
               FROM   PREV_MONTH_DISENROLL B
               WHERE  (B.MONTH_ID BETWEEN TO_CHAR( ADD_MONTHS( TO_DATE( m.MONTH_ID, 'YYYYMM'), -2), 'YYYYMM') AND m.MONTH_ID
    ...
    (SELECT   m.month_id,
              ...
              (SELECT SUM(N_Mbrs_PREV)
               FROM   PREV_MONTH_DISENROLL B
               WHERE  (B.MONTH_ID BETWEEN TO_CHAR( ADD_MONTHS( TO_DATE( m.MONTH_ID, 'YYYYMM'), -2), 'YYYYMM') AND m.MONTH_ID
                      ) AND b.LOB = m.LOB AND b.county = m.county AND m.region = m.region)
                  AS N_Mbrs_disenrollees3,
              (SELECT ROUND( SUM(Risk_score) / SUM(N_Mbrs_PREV), 3)
               FROM   PREV_MONTH_DISENROLL B
               WHERE  (B.MONTH_ID BETWEEN TO_CHAR( ADD_MONTHS( TO_DATE( m.MONTH_ID, 'YYYYMM'), -2), 'YYYYMM') AND m.MONTH_ID
